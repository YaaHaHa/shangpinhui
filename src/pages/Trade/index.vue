  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        ...
        >
        <p @click="chooseReceiver(address)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ sendInfo.userAddress }}</span>
        收货人：<span>{{ sendInfo.consignee }}</span>
        <span>{{ sendInfo.phoneNum }}</span>
      </div>
    </div>
...
import { mapGetters, mapState } from 'vuex';
  export default {
    /* 
      ...
      今天：当本地有token，但是没有数据userInfo的时候，一直循环dispatch，
          (错误的认知：原因：await store.dispatch('userInfo/requserout');，没加await
          async与await一定要记得啊啊啊！！，下次先await在async)
          仍没解决
    ...
    */
    ...
    methods: {
      // 请求跳转到支付页面，根据请求的结果决定是否跳转
      
      async toPay(){
        // 配置请求数据
        let tradeNo = this.tradeInfo.tradeNo;
        let subTrade ={
          consignee:this.sendInfo.consignee,
          consigneeTel:this.sendInfo.phoneNum,
          deliveryAddress: this.sendInfo.userAddress,
          paymentWay: "ONLINE",
        ...
        // 抓出请求的错误处理，成功就跳转
        try {
          const result = await this.$api.reqSubTrade(tradeNo,subTrade);
          if (result.code === 200){
            alert("提交成功，准备跳转支付界面");
      ...
      // 请求页面数据，因为这里不需要根据请求回来的结果再下一步，所以不需要async然后await等他的结果，他会去执行action
      // 然后存在state里的
      requserInfo (){
        
        this.$store.dispatch('trade/reqtrade');
      },
      // 选择收货人，排他心理
