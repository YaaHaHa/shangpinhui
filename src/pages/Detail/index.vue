    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgaeList="imgaeList"/>
          <!-- 小图列表 -->
          <ImageList :imgaeList="imgaeList"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="40" 
                  ...
                  @click="changeChecked(saleAttrValue,spuSaleAttr.spuSaleAttrValueList)">
...
import { mapGetters } from 'vuex'
  ...
  export default {
    ...
    //我们一般在beforeMount当中去处理数据，处理完成后后期就可以直接使用
    beforeMount() {
    ...
    methods: {
      getSkuDetailInfo(){
        this.$store.dispatch("detail/reqdetailInfo",this.skuId);

      },
      // 采用排异的实现处理
      changeChecked(spuSaleAttrvalue,spuSaleAttrValueList){
        // 所有的属性都变白
        spuSaleAttrValueList.forEach(spuSaleAttrValueItem =>  spuSaleAttrValueItem.isChecked = "0"
        )
        // 点击的这个属性变绿
